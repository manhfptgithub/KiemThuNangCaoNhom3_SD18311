 ...
 * @author ADMIN
 */
public class ChiTietSanPhamDAO extends DuAn1DAO1<ChiTietSanPham, String> {
    ...
    public List<ChiTietSanPham> getTheoThuocTinh(String tenMauSac, String tenSize,String trangThai,String maSP) {
        String sql = "";
        List<ChiTietSanPham> list = new ArrayList<>();
        if(trangThai.equals("All")){
            sql = "SELECT * FROM tblChiTietSanPham CTSP JOIN tblMauSacSP MS ON CTSP.MaMauSac = MS.MaMauSac JOIN tblSize Sz ON CTSP.MaSize = Sz.MaSize\n"
                + "WHERE MS.TenMauSac LIKE ? AND SZ.tenSize LIKE ? AND CTSP.MaAoKhoac=?";
        }
        if(!trangThai.equals("All")){
            sql = "SELECT * FROM tblChiTietSanPham CTSP JOIN tblMauSacSP MS ON CTSP.MaMauSac = MS.MaMauSac JOIN tblSize Sz ON CTSP.MaSize = Sz.MaSize\n"
                + "WHERE MS.TenMauSac LIKE ? AND SZ.tenSize LIKE ? AND CTSP.MaAoKhoac=? AND CTSP.TrangThai = ?";
        }
        try {
